...
const STORAGE_FILE = './lowest-glyphs.json'
...
if (!storage.lowestGlyphs) {
    storage.lowestGlyphs = {}
}
...
async function processBatch(inscriptions) {
    ...
    for (const result of results) {
        ...
        //console.log(it)
        if (isValidGlyph(text)) {
            console.log(`\n\n\n~~~~~~~~~~\n\n\nFound glyph: ${text}\n\n\n~~~~~~~~~~\n\n\n`)
            if (!storage.lowestGlyphs[text] || it.num < storage.lowestGlyphs[text].num) {
                storage.lowestGlyphs[text] = {
                    id: it.id,
                    num: it.num
                }
            }
